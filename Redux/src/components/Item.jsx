...
function Item(props) {
  const showElementLevel = (level) => {
    let elemLevel = (
      <h5>
        <span className="badge badge-pill badge-warning">Small</span>
      </h5>
    );
    if (level === 1) {
      elemLevel = (
        <h5>
          <span className="badge badge-pill badge-success">Medium</span>
        </h5>
      );
    } else if (level === 2) {
      elemLevel = (
  ...
  return (
    <tr>
      ...
      <td>{item.name}</td>
      <td>{showElementLevel(item.level)}</td>
      <td>
        <a
...
const mapDispatchToProps = (dispatch, props) => {
  return {
    ...
    onClickDelete: (id) => {
      dispatch(actions.fetchDeleteTasksRequest(id));
    },
    onEditTask: (task) => {
